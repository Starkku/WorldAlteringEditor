...
public sealed class ScriptingChangedEventArgs : EventArgs
{
    public ScriptingChangedEventArgs(IEnumerable<Type> affectedModelTypes)
    {
        ArgumentNullException.ThrowIfNull(affectedModelTypes);
        this.affectedModelTypes = new HashSet<Type>(affectedModelTypes);
    }

    private readonly HashSet<Type> affectedModelTypes;

    public IReadOnlyCollection<Type> AffectedModelTypes => affectedModelTypes;

    public bool Affects<TModel>() => affectedModelTypes.Contains(typeof(TModel));

    public bool AffectsAny(params Type[] modelTypes)
    {
        ArgumentNullException.ThrowIfNull(modelTypes);
        return modelTypes.Any(affectedModelTypes.Contains);
    }
}
...
public class Map : IMap
{
    ...
    /// <summary>
    /// Raised after one or more map scripting elements have been modified.
    /// </summary>
    public event EventHandler<ScriptingChangedEventArgs> ScriptingChanged;
    ...
    public void NotifyScriptingChanged(IEnumerable<Type> affectedModelTypes)
    {
        ScriptingChanged?.Invoke(this, new ScriptingChangedEventArgs(affectedModelTypes));
    }
    ...
    private void EnsureTechnoObjectId(TechnoBase techno)
    {
        ...
        {
            techno.ObjectId = nextTechnoObjectId;
            nextTechnoObjectId++;
        }
    }
