    ...
    public class SelectScriptActionPresetOptionWindow : SelectObjectWindow<ScriptActionPresetOption>
    {
        ...
        public string FillPresetOptions(ScriptActionEntry entry, ScriptAction action)
        {
            ...
            if (action.ParamType == TriggerParamType.LocalVariable)
            {
                ...
                {
                    presetOptions.Add(new ScriptActionPresetOption() { Value = i, Text = map.LocalVariables[i].Name });
                }
            }
            else if (action.ParamType == TriggerParamType.Waypoint)
            {
                ...
                {
                    presetOptions.Add(new ScriptActionPresetOption() { Value = waypoint.Identifier, Text = waypoint.Identifier.ToString() });
                }
            }
            else if (action.ParamType == TriggerParamType.HouseType)
            {
                ...
                {
                    presetOptions.Add(new ScriptActionPresetOption() { Value = houseType.Index, Text = houseType.ININame, Color = Helpers.GetHouseTypeUITextColor(houseType) });
                }
            }
            else if (action.ParamType == TriggerParamType.House)
            {
                ...
                {
                    presetOptions.Add(new ScriptActionPresetOption() { Value = house.ID, Text = house.ININame, Color = Helpers.GetHouseUITextColor(house) });
                }
            }
